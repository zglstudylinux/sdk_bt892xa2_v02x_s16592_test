#include "include.h"
    ...
    static dmns_cb_t dmns_cb AT(.nr_buf.dmdnn_sta);
    u8 cfg_micr_phase_en;
...
void bt_sco_dump_init(u8 *sysclk, bt_voice_cfg_t *p)
{
    nr_cb_t *nr = &p->nr;
    if (xcfg_cb.huart_en) {
#if BT_SCO_DUMP_EN && BT_SCO_DNN_EN && BT_SCO_AGC_EN
        nr->dump_en = DUMP_SNDP_TALK | DUMP_SNDP_NR | DUMP_AGC_OUTPUT;
#elif BT_SNDP_DUMP_EN || BT_BCNS_DUMP_EN || BT_DMNS_DUMP_EN || BT_AINS2_DUMP_EN || BT_DNN_DUMP_EN || BT_SCO_DUMP_EN || BT_AINS3_DUMP_EN
        nr->dump_en = DUMP_SNDP_TALK | DUMP_SNDP_FF | DUMP_SNDP_NR;
#elif BT_AEC_DUMP_EN
        nr->dump_en = DUMP_AEC_INPUT | DUMP_AEC_FAR | DUMP_AEC_OUTPUT;
#elif BT_AEC_FRE_DUMP_EN
        if (xcfg_cb.micl_en && xcfg_cb.micr_en) {
            nr->dump_en = DUMP_SNDP_TALK | DUMP_SNDP_FF | DUMP_AEC_FAR;     //双麦
        } else {
            nr->dump_en = DUMP_SNDP_TALK | DUMP_SNDP_NR | DUMP_AEC_FAR;     //单麦
        }
...
#if BT_NLMS_AEC_EN
    *sysclk = *sysclk < SYS_160M ? SYS_160M : SYS_120M;
#elif (BT_NLMS_FRE_EN && BT_SCO_FAR_NR_EN)
    *sysclk = *sysclk < SYS_160M ? SYS_160M : SYS_120M;
#else
    *sysclk = SYS_120M;
#endif
...
//	aiaec_dnn_cb.exp_range_L			   	= 0;
    ...
    *sysclk = *sysclk < SYS_120M ? SYS_120M : SYS_120M;
}
#endif

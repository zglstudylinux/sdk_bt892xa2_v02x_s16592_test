...
#define SPIFLASH_MP3_LOOPBACK_EN        0   //外部或内部flash音乐循环
...
#define BT_AEC_EN                       0                           //是否打开硬件频域AEC算法

#define BT_NLMS_AEC_EN                  0                           //是否打开NLMS_AEC算法,此功能打开原始AEC失效
#define BT_NLMS_NLP_MODE                0                           //1：改进回声抑制和双讲效果（BT_NLMS_AEC_EN置1有效）

#define BT_NLMS_FRE_EN                  1                           //是否打开频域NLMS AEC，需要与降噪算法同时打开，注：BT_SCO_ANS_EN, BT_SCO_AINS2_EN 算法不支持该AEC
#define BT_NLMS_FRE_REF_SET             0                           //0:NLP参考AEC前(DF)+线性后(EF)，1:NLP只参考AEC前(DF)，2:NLP只参考AEC后(EF)
...
#define BT_SNDP_EN                      1       //是否打开声加单麦降噪算法
...
#define AUX_2_SDADC_EN                  0                           //AUX是否进SDADC, 否则直通DAC。进SDADC可以调AUX EQ, DAC DRC及AUX录音等功能
...
#define DAC_DRC_EN                      0                           //是否使能DRC功能（目前只支持AAC、SBC和aux），需要把时钟调到至少48M，DRC参数在dac.drc
...
#define CHARGE_BOX_LK_CRC_EN            0            //是否使能TWS Linkkey信息校验，智能充电仓建议打开，但VHOUSE_CMD_GET_TWS_BTADDR命令会加多2byte内容，注意前后兼容

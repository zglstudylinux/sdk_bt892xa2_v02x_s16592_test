...
static u8 bt_music_audio_sta AT(.sco_data);
#endif
#if ASR_EN
static u8 bt_asr_sta;
#endif

extern const u16 btmic_ch_tbl[];
extern s16 bt_sco_dump_buf[4][256];
void sdadc_set_digital_gain(u8 gain);
void bt_sco_rec_exit(void);
u8 dac_get_sco_anl_vol(void);

#if BT_CALL_SWITCH_WAV_CACHE_EN
#define DAC_OBUF1_SIZE      144
extern u32 dac_src1_obuf[DAC_OBUF1_SIZE];
extern u32 *cfg_dac_src1_obuf;
extern u16 cfg_dac_obuf1_size;
u32 dac_src1_call_obuf[DAC_OBUF1_SIZE * 2] AT(.nr_buf.wav);
#endif
...
void sco_smic_switch(u8 smic_on)
{
    ...
    if(smic_on == '1'){
        ...
        nr->type = (nr->type & ~0x07) | NR_CFG_TYPE_AINS3;
    }else{
...
static void bt_sco_eq_init(void)
{
    ...
    bt_voice_cfg.nr.sco_fade_en = BT_SCO_FADE_EN;
}
...
static void bt_call_alg_init(void)
{
	...
	///通话舒适噪声算法初始化
	bt_sco_comfort_noise_init(&sysclk, &bt_voice_cfg.nr);

	///通话AGC算法初始化
	bt_sco_agc_init(&sysclk, &bt_voice_cfg.nr);
...
#if BT_MUSIC_AUDIO_EN
    bt_music_audio_sta = bt_music_audio_get_state();
    bt_music_audio_set_state(MUSIC_AUDIO_NONE);
#endif

#if BT_CALL_SWITCH_WAV_CACHE_EN
    cfg_dac_src1_obuf = dac_src1_call_obuf;
    cfg_dac_obuf1_size = DAC_OBUF1_SIZE * 2;
#endif
...
#if BT_CALL_SWITCH_WAV_CACHE_EN
    cfg_dac_src1_obuf = dac_src1_obuf;
    cfg_dac_obuf1_size = DAC_OBUF1_SIZE;
#endif

#if BT_MUSIC_AUDIO_EN
    bt_music_audio_init(BT_MUSIC_AUDIO_TYPE);
    bt_music_audio_set_state(bt_music_audio_sta);
#endif
#if BT_DYNAMIC_BASS_BOOST_EN

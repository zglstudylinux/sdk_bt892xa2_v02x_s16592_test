...
void func_bt_esbc_res_play(u32 addr, u32 len)
{
    if (len == 0) {
        return;
    }
...
void func_bt_process(void)
{
    ...
    if(f_bt.disp_status >= BT_STA_INCOMING){
        if(f_bt.disp_status == BT_STA_OTA){
            func_bt_ota_process();
        }else{

...
#if ASR_EN
    #if !ASR_VAD_EN
    return;                             //没开VAD不进sniff mode
    #endif
    if (sys_cb.asr_enable && is_vad_wake()) {
        return;                         //VAD已经唤醒，不进sniff mode
    }
#endif
...
#if ASR_EN
    vad_enable = bsp_asr_get_statue() && sys_cb.asr_enable;
#endif
...
#endif
    while(bt_is_in_sleep()) {
        WDT_CLR();
#if ASR_EN
        sys_cb.sys_is_sleep = 1;
#endif
...
#endif
    sys_cb.sys_is_sleep = 0;
#endif
    printf("sleep_exit\n");
}

ALIGNED(512)
AT(.text.lowpwr.sleep_process)
...
void sfunc_pwrdown_do(u8 vusb_wakeup_en)
{
    ...
    if ((RTCCON3 & BIT(12)) || sys_cb.lowbat_flag) {
        tkey_wakeup_en = 0;                         //低电或者仓内关机, 关掉触模唤醒
    }
    if (tkey_wakeup_en) {
...
#if CHARGE_EN
    if (((xcfg_cb.charge_en) || (xcfg_cb.charge_dc_reset)) && (vusb_wakeup_en)) {
...
#endif // USER_PWRKEY
#if VUSB_TBOX_QTEST_EN
    if(sys_cb.pdn_boat_flag) {  //若船运模式关机，只保留5V唤醒

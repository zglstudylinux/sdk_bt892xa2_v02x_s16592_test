...
bool get_adc_val(void)
{
    ...
    if (mic_bias_trim_is_en()) {
        ...
        adc_cb.vusb_val = (u16)(adc_cb.sfr[ADCCH_VUSB]);
    }
...
#if VUSB_DETECT_EN
u32 get_vusb_val(void)
{
    return (u32)adc_cb.vusb_val * 3 * ((u32)VBG_VOLTAGE >> 1) / adc_cb.bg;
}
#endif // VUSB_DETECT_EN

u32 get_mic_dc_volt(void)
{
    return (u32)adc_cb.mic_dc_val * ((u32)VBG_VOLTAGE >> 1) / (adc_cb.bg >> 2);
}
...
bool is_powron_frist_enable(u32 rtccon9)
{
    ...
    if (rtccon9 & BIT(0)) {                                        //rtc alarm wakeup
        return false;
    }

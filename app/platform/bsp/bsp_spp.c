...
void bt_app_cmd_process(u8 *ptr, u16 size, u8 type)
{
    ...
    if (xcfg_cb.huart_en || xcfg_cb.eq_dgb_spp_en || ANC_EN) {
...
#if EQ_TBL_RES2_EN
            if ((ptr[2] == '?')||((ptr[2] == 'S')&&((ptr[3] == 'P')||(ptr[3] == 'F')))) {
#else
            if (ptr[2] == '?'){
#endif
        ...
        #if DAC_DRC_EN
        if (ptr[0] == 'D' && ptr[1] == 'R') {       //DRC消息
            memcpy(eq_rx_buf, ptr, size);
            msg_enqueue(EVT_ONLINE_SET_DRC);
            return;
        }
        #endif
    }
...
void spp_custom2_rx_callback(uint8_t *packet, uint16_t size)
{
    app_spp2_rx_callback(packet, size);
}

void spp_custom1_rx_callback(uint8_t *packet, uint16_t size)
{
#if GFPS_EN
    google_fast_pair_spp_recv_proc(packet, size);
#endif

    app_spp1_rx_callback(packet, size);
}

void spp_rx_callback(uint8_t *packet, uint16_t size)
{
#if BT_SPP_FOT_EN
    if(fot_app_connect_auth(packet, size, FOTA_CON_SPP)){
    ...
    app_spp_rx_callback(packet,size);
}
...
void spp_disconnect_callback(void)
{
    ...
    app_spp_disconnect_callback();
}

void spp_custom1_connect_callback(void)
{
    printf("--->spp_custom1_connect_callback\n");

#if GFPS_EN
    google_fast_pair_spp_connected_callback();
#endif

    app_spp1_connect_callback();
}

void spp_custom1_disconnect_callback(void)
{
    printf("--->spp_custom1_disconnect_callback\n");

    app_spp1_disconnect_callback();
}


void spp_custom2_connect_callback(void)
{
    printf("--->spp_custom2_connect_callback\n");

    app_spp2_connect_callback();
}

void spp_custom2_disconnect_callback(void)
{
    printf("--->spp_custom2_disconnect_callback\n");

    app_spp2_disconnect_callback();
}

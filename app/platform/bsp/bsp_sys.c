...
#if (ANC_EN && ANC_SOFT_FADE_EN)
    bsp_anc_soft_ancvol_detect();
#endif
}
...
//    printf("bat level: %d %d\n", sys_cb.vbat, bat_level);
    return (bat_level > 100 ? 100 : bat_level);
#else
    return 100;
#endif
}
...
#if !USER_TKEY
    tkey_sw_reset();            //释放tkey的四个channel
#endif
...
void tws_lr_xcfg_sel(void)
{
    ...
    if (8 == xcfg_cb.bt_tws_lr_mode) { //有PWRKEY 820K接地则为左声道
        ...
        sys_cb.tws_left_channel = sys_cb.vusb_force_channel - 1;
    } else {
        return;
    }
...
//{
//    //打开定时器时钟：timer1:CLKGAT0[24], timer2:CLKGAT0[25], timer3:CLKGAT1[8], timer4:CLKGAT1[9], timer5:CLKGAT1[10]
//    CLKGAT1 |= BIT(8);
//    delay_us(2);    //开时钟后一定要延时一点时间
//    TMR3CPND = BIT(16);
...
//    sys_irq_init(IRQ_TMR3_VECTOR, 0, timer3_isr);   //如果使用高优先级中断，中断函数前要加FIQ关键字
//}

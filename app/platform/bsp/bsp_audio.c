...
void asr_sdadc_process(u8 *ptr, u32 samples, int ch_mode);
...
#elif BT_SNDP_EN || BT_SNDP_DMIC_EN || BT_SCO_AINS2_EN || BT_SCO_AINS3_EN || BT_SCO_DMNS_EN || BT_SCO_DNN_EN || BT_SCO_DMDNN_EN || BT_SCO_LDMDNN_EN || BT_SNDP_DMIC_DNN_EN || BT_SCO_AIAEC_DNN_EN
    ...
    #if !SDADC_DRC_EN && !SDADC_SINGLE_DRC_EN
    WEAK uint8_t cfg_sdadc_drc_mode = 0;
    WEAK uint8_t cfg_sdadc_drc_en = 0;
    #endif
    #if SDADC_DRC_EN                              //开启ADC DRC
    uint8_t cfg_sdadc_drc_en = 1;
    uint8_t cfg_sdadc_drc_mode = 0;
    #endif
    #if SDADC_SINGLE_DRC_EN
    uint8_t cfg_sdadc_drc_en = 1;
    uint8_t cfg_sdadc_drc_mode = 2;
    #endif
#endif
...
/*   通道,             增益,        采样率,      通路控制,    样点数,   回调函数*/
	...
	{MIC_CHANNEL_CFG,  (12 << 6),   SPR_16000,   ADC2DAC_EN,    128,    opus_sdadc_callback},           /* opus  */
    ...
    {MIC_CHANNEL_CFG,  (8 << 6),    SPR_16000,   ADC2ASR_EN,    256,    asr_sdadc_callback},            /* ASR     */
};
...
{
#if BT_SNDP_DMIC_EN || BT_SCO_BCNS_EN || BT_SCO_DMNS_EN || BT_SCO_DMDNN_EN || BT_SCO_LDMDNN_EN || BT_SNDP_DMIC_DNN_EN
    if (xcfg_cb.micl_en && xcfg_cb.micr_en) {
        #if TINY_TRANSPARENCY_EN
        if (!sys_cb.ttp_start)
        #endif
...
#endif // FUNC_AUX_EN
    if (path_idx == AUDIO_PATH_BTMIC || path_idx == AUDIO_PATH_KARAOK || path_idx == AUDIO_PATH_BTVOICE || path_idx == AUDIO_PATH_TTP || path_idx == AUDIO_PATH_OPUS) {
...
#if SDADC_DRC_EN
    sdadc_drc_init((u8 *)RES_BUF_EQ_SDADC_DRC, (s32 )RES_LEN_EQ_SDADC_DRC);
#elif SDADC_SINGLE_DRC_EN
    sdadc_drc_init((u8 *)RES_BUF_EQ_SINGLE_DRC, (s32 )RES_LEN_EQ_SINGLE_DRC);
#endif
...
#endif // FUNC_AUX_EN
    if (path_idx == AUDIO_PATH_BTMIC || path_idx == AUDIO_PATH_KARAOK || path_idx == AUDIO_PATH_BTVOICE || path_idx == AUDIO_PATH_TTP || path_idx == AUDIO_PATH_OPUS || path_idx == AUDIO_PATH_ASR) {
        cfg.channel = bt_mic_channel_get();
    }

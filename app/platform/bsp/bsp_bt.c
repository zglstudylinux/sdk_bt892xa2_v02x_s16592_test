...
{
    //更新配置工具的设置
    cfg_bt_rf_def_txpwr = xcfg_cb.bt_rf_param_en? 0 : xcfg_cb.bt_rf_pwrdec;
    ...
    if(xcfg_cb.bt_tws_en == 0) {
        ...
        cfg_bt_tws_not_auto_connect = BT_TWS_NOT_AUTO_CONNECT_EN;
		cfg_bt_hci_disc_only_spp = BT_HCI_DIS_ONLY_SPP_EN;
    }
#endif // BT_TWS_EN
    ...
    if(cfg_bt_work_mode == MODE_FCC_TEST) {
        bt_fcc_init();
    }
#endif
...
void bsp_bt_evt_emit(uint evt, void *params)
{
...
//        break;
    ...
    case BT_NOTICE_MUSIC_CHANGE_DEV:
        ...
        if ((packet[1]&BIT(1)) == 0) { // bit(1) ：初次连接配对不用恢复音量
            ...
            if (update) {
#if BT_2ACL_EN
                if (xcfg_cb.bt_2acl_en && (bt_get_connected_num() > 1) && (bt_get_status() == BT_STA_PLAYING)) {            // 两个设备互相切换，播放时才设置音量
...
#if LE_DUEROS_DMA_EN
    if (dueros_dma_notify_state_number_get()) {
        printf("!!!The number of notification states is not empty, delayed entry charge!!!\n");
        sys_cb.dc_rst_flag = 1;
        return;
    } else {
        printf("---The number of notification states is empty, allow in charge---\n");
    }
#endif

...
bool bsp_res_music_play(u32 addr, u32 len, res_play_kick_func res_play_kick)
{
    ...
    return res_play_kick(addr, len, !bt_tws_is_connected());
}
...
void wav_res_play(u32 addr, u32 len)
{
    ...
    if (music_soft_vol_get_state()) {
        ...
        bsp_change_volume(sys_cb.vol);  //调回系统音量
    }
#endif // BT_MUSIC_EFFECT_SOFT_VOL_EN
...
#if ASR_EN
    case TWS_RES_ASR_ON:
        *addr = RES_BUF_ASR_ON_MP3;
        *len = RES_LEN_ASR_ON_MP3;
        break;

    case TWS_RES_ASR_OFF:
        *addr = RES_BUF_ASR_OFF_MP3;
        *len = RES_LEN_ASR_OFF_MP3;
        break;
#endif
    default :
        *addr = 0;

...
u8 cfg_anc_dc_adj_enable = ANC_EXP_DC_ADJ_EN;
...
void bsp_anc_init(void)
{
    ...
    anc_soft_fade_init(ANC_SOFT_FADE_EN, ANC_FADE_IN_STEP, ANC_FADE_OUT_STEP);

#if ANC_FADE_IN_EN
    anc_fade_enable(ANC_FADE_IN_EN, FADE_IN_STEP);
#endif
...
#if ANC_SOFT_FADE_EN
u8 anc_soft_loc_sta;                //用于ANC软件淡入淡出时，同步状态
u8 anc_soft_rem_sta;

//ANC软件淡入淡出的相关函数
AT(.text.bsp.anc.soft)
void bsp_anc_soft_set_mode(u8 mode)
{
    anc_soft_set_mode(mode);
}

AT(.text.bsp.anc.soft)
void bsp_anc_soft_stop(void)
{
    bsp_anc_stop();
}

AT(.text.bsp.anc.soft)
void bsp_anc_soft_start(void)
{
    bsp_anc_start();
}

AT(.text.bsp.anc.soft)
void bsp_anc_soft_set_param(u8 tp)
{
    bsp_anc_set_param(tp);
}


AT(.text.func.process) WEAK
void bsp_anc_soft_mode_detect(void)
{
    anc_soft_mode_detect();
}

AT(.com_text.anc_soft) WEAK
void bsp_anc_soft_ancvol_detect(void)
{
    anc_soft_vol_fade_process();
}

AT(.text.func.process) WEAK
void anc_soft_mode_sync(u8 loc_sta)
{
    anc_soft_loc_sta = loc_sta;
    bt_tws_sync_setting();
}

#else
void anc_soft_stop_do(void) {}
void bsp_anc_soft_stop(void) {}

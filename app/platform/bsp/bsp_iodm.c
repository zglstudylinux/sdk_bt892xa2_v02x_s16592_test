...
AT(.com_text.iodm)
void bsp_iodm_packet_dma_recv(u8* rxbuf)
{
    if(memcmp(rxbuf,vhouse_header,3) == 0){
        vhouse_cb.rx_flag = 1;
    }
}
#endif
...
void bsp_iodm_init(void)
{
    memset(&iodm_packt,0,sizeof(vh_packet_t));
#if !VUSB_HUART_DMA_EN
    bsp_vusb_uart_init();
#endif
}

WEAK void iodm_process(void)
{
     vh_packet_t *packet = &iodm_packt;
#if VUSB_HUART_DMA_EN

...
void bt_call_test_set_mic(u8 ch);
...
bool rmk_save_file_param(u8 *buf)
{
    ...
    if(i == rmk_cb.rescnt){
        rmk_cb.err = RMK_ERR_RES_FILE_NOT_FIND;
    }
    return false;
}
...
uint16_t tws_get_spp_eq_info(uint8_t *buf)
{
    ...
    u16 size = EQ_BUFFER_LEN;
#if DAC_DRC_EN
    if (buf[0] == 'D' && buf[1] == 'R') {
        size = little_endian_read_16(buf, 4);
        size += 6;
    }
#endif
//    printf("get_spp_eq: %d\n", size);
...
void drc_tx_ack(u8 coef, u8 tag)
{
    u8 ack[4];
    ack[0] = 'D';
    ack[1] = coef;
...
void bsp_drc_parse_cmd(void)
{
    ...
    u8 coeff_num = eq_rx_buf[6];
    u32 size = little_endian_read_16(eq_rx_buf, 4);
    u32 crc = calc_crc(eq_rx_buf, size + 4, EQ_CRC_SEED);
    if (crc == little_endian_read_16(eq_rx_buf, size + 4)) {
        drc_tx_ack(coeff_num, 0);               //ok
    } else {
        drc_tx_ack(coeff_num, 1);               //fail
        return;
    }
...
#endif
        dac_drc_set_param((const void*)&eq_rx_buf[10]);
    }
}
#endif
...
static struct eq_cfg_t eq_cfg;
u32 eq_overall_gain = 0x800000;     //0db
...
{
   music_set_eq(10, &eq_overall_gain);
}
...
void music_set_eq_overall_gain(int gain)
{
    switch(gain){
        case 3:
            eq_overall_gain = 0xb4ce08;
            break;
        case 2:
            eq_overall_gain = 0xa12478;
            break;
        case 1:
            eq_overall_gain = 0x8f9e4d;
            break;
        case 0:
            eq_overall_gain = 0x800000;
            break;
        case -1:
            eq_overall_gain = 0x721483;
            break;
        case -2:
            eq_overall_gain = 0x65ac8c;
            break;
        case -3:
            eq_overall_gain = 0x5a9df8;
            break;
        case -4:
            eq_overall_gain = 0x50c336;
            break;
        case -5:
            eq_overall_gain = 0x47facd;
            break;
        case -6:
            eq_overall_gain = 0x4026e7;
            break;
        case -7:
            eq_overall_gain = 0x392cee;
            break;
        case -8:
            eq_overall_gain = 0x32f52d;
            break;
        case -9:
            eq_overall_gain = 0x2d6a86;
            break;
        case -10:
            eq_overall_gain = 0x287a27;
            break;
        default:
            break;
    }

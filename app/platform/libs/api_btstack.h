...
#define HFP_1T2_CALL_HOLD                   0x80    //是否支持一拖二通话hold
...
#define A2DP_AVRCP_RESET_VOL_CTRL           0x01    //手机支持同步音量时，首次配对连接是否恢复到最大音量
#define A2DP_AVDTP_1T2_FORCE_SWITCH         0x02    //1拖2时是否支持音乐强制切换
#define A2DP_AVRCP_1T2_RECORD_DEVICE_VOL    0x04    //1拖2时非活动手机调整音量是否同步

//一拖二特性
#define BT_2ACL_PAIR_ODEV               0x01    //是否支持连接两部手机配对时仅支持连接一部手机
#define BT_2ACL_CALL_INCOME_FOR_PLAY    0x04    //来电是否接听当前播放的手机
#define BT_2ACL_CALL_TERMINATE_OPT1     0x08    //A通话中，B来电，按下挂断B
#define BT_2ACL_A2DP_SWITCH_TO          0x10    //A2DP切换播放暂停增加超时
...
#define TWS_FEAT_XOR_ACC                0x0800  //XOR_ACC
#define TWS_FEAT_PUBLIC_ADDR            0x1000  //TWS连接后主耳使用公共地址连接手机
...
enum {
    ...
    BT_CTL_HFP_TERMINATE_ACTIVE,
    BT_CTL_HFP_ANSWER_ACTIVE,
    BT_CTL_GET_PLAY_STATUS_INFO,
    BT_CTL_HFP_CONNECT,
    BT_CTL_HFP_DISCONNECT,
    BT_CTL_ATT_CONNECT,
    BT_CTL_ATT_DISCONNECT,
    BT_CTL_MAX,
...
extern uint8_t cfg_bt_hid_douyin_mode;
extern uint8_t cfg_bt_hid_android_param;
...
#define bt_hid_is_douyin_type()                 cfg_bt_hid_douyin_mode
...
void bt_hid_send_comb_key(uint key1, uint key2);
...
bool bt_is_in_sleep(void);                      //判断蓝牙是否进入休眠状态
...
bool bt_is_silence(void);                       //一拖二时判断另一部手机是否静音
bool bt_cur_dev_is_silence(void);               //判断当前设备是否静音
...
uint8_t bt_get_connect_status(u8 *bd_addr);     //获取当前地址设备的连接状态，已连接返回BT_STA_CONNECTED，连接中BT_STA_CONNECTING，未连接BT_STA_DISCONNECTING
...
bool bt_tws_get_link_info_key(uint8_t *bd_addr, uint8_t *link_key);         //根据tws配对地址bd_addr，读取16byte link_key
...
void bt_nor_delete_link_info_by_addr(uint8_t *bd_addr, bool ext);	//根据地址删除连接信息
...
uint32_t bt_tws_master_clock_get(void);         //获取tws master的clock
...
#define bt_nor_acl_is_connected()               (bool)(bt_tws_get_status() & TWS_STA_NOR_ACL_FLAG)
...
#define bt_music_get_id3_tag()                  bt_ctrl_msg(BT_CTL_ID3_GET_ELE_ATT);    //主动查询id3 tag信息
...
#define bt_hfp_send_at_cmd()                    bt_ctrl_msg(BT_CTL_HFP_AT_CMD)          //发送HFP AT指令, 通过hfp_get_at_cmd回调获取指针
...
#define bt_tws_vol_change()                     bt_ctrl_msg(BT_CTL_TWS_VOL_CHANGE)      //通知对耳音量已调整
...
#define bt_tws_sync_custom_data()               bt_tws_msg(TWS_SYNC_INFO, SYNC_INFO_CUSTOM_DATA)
...
#define dueros_ai_speech_stop()                 bt_ctrl_msg(BT_CTL_DUEROS_AI_STOP)                 //未支持
#define dueros_dma_key_msg_ctl()                bt_ctrl_msg(BT_CTL_DUEROS_DMA_CTL)                 //同步从机key msg

//BLE
#define ble_adv_dis()                           bt_ctrl_msg(BT_CTL_BLE_ADV_DISABLE)
#define ble_adv_en()                            bt_ctrl_msg(BT_CTL_BLE_ADV_ENABLE)
#define ble_enable_adv()                        btstack_ble_adv_en()                //蓝牙线程才可直接调用该函数
#define ble_disable_adv()                       btstack_ble_adv_dis()               //蓝牙线程才可直接调用该函数
...
bool a2dp_is_avctp_connect(void);
...
extern struct txpkt_tag spp_custom1_tx;
extern struct txpkt_tag spp_custom2_tx;
...
bool spp_default_connected(void);
int bt_spp_custom1_tx(uint8_t *packet, uint16_t len);
bool spp_custom1_connected(void);
int bt_spp_custom2_tx(uint8_t *packet, uint16_t len);
bool spp_custom2_connected(void);
...
#define bt_hid_mouse(keycode)            		bt_hid_send_key(HID_MOUSE, keycode)          		//鼠标
#define bt_hid_douyin(keycode)                  bt_hid_send_key(HID_DOUYIN, keycode)                //抖音
...
int latt_tx_notify(uint8_t index, const uint8_t *value, uint16_t value_len);
void bt_latt_connect(void);
void bt_latt_disconnect(void);
...
void btstack_ble_adv_en(void);
void btstack_ble_adv_dis(void);

...
typedef struct {
    s32 outdat;     //A = outdat / 2^31
    s32 outexp;     //B = 2^outexp
...
//硬件1维矩阵运算
//原型：for (int i = 0; i < loop; i++) sum += x0 * y0;
//约束条件：x0地址范围0x55800, sum、y0地址范围0x28000~0x40000, loop 4byte对齐并且小于等于512, 通话运行声加算法下不能使用
void matrix_hw(int32_t *sum, const int8_t *x0, const int8_t *y0, int loop);

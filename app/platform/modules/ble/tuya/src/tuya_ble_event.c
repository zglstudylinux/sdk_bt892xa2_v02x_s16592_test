...
AT(.tuya_data)
uint32_t TUYA_BLE_SCHED_BUF[CEIL_DIV(TUYA_BLE_SCHED_BUF_SIZE((TUYA_BLE_EVT_SIZE), (TUYA_BLE_EVT_MAX_NUM)),sizeof(uint32_t))];    
...
void tuya_ble_event_queue_init(void)
{
    ...
    do
    {
        tuya_ble_sched_init((TUYA_BLE_EVT_SIZE), (TUYA_BLE_EVT_MAX_NUM), TUYA_BLE_SCHED_BUF);
    } while (0);

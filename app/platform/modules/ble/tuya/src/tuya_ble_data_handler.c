...
AT(.tuya_data)
static uint32_t tuya_ble_firmware_version;
AT(.tuya_data)
static uint32_t tuya_ble_hardware_version;

AT(.tuya_data)
static uint32_t tuya_ble_mcu_firmware_version;
AT(.tuya_data)
static uint32_t tuya_ble_mcu_hardware_version;
...
uint8_t tuya_ble_pair_rand[6] AT(.tuya_data);
uint8_t tuya_ble_pair_rand_valid AT(.tuya_data);
uint32_t tuya_ble_receive_sn AT(.tuya_data);
uint32_t tuya_ble_send_sn = 1;

tuya_ble_ota_status_t tuya_ble_ota_status AT(.tuya_data);
...
void tuya_ble_commonData_rx_proc(uint8_t *buf,uint16_t len)
{
    ...
    {
        tuya_ble_receive_sn = current_sn;
    }
...
static void tuya_ble_handle_unix_time_char_ms_resp(uint8_t*recv_data,uint16_t recv_len)
{
    ...
    if(!buffer_value_is_all_x(&recv_data[13],13,0))
    {
        ...
        time_stamp_ms = atoi(current_timems_string);
...
static void tuya_ble_handle_unix_time_with_dst_date_resp(uint8_t*recv_data,uint16_t recv_len)
{
    ...
    if(!buffer_value_is_all_x(&recv_data[13],10,0))
    {
        ...
        time_stamp = atoi(current_time_string);
...
bt_state_info_t bt_state_info AT(.tuya_data);
...
static void tuya_ble_handle_bt_state_req(uint8_t*recv_data,uint16_t recv_len)
{
    ...
    for(u8 i=0;i<6;i++){
        bt_state_info.mac[i] = bt_addr[5-i];
    }

    if(bt_nor_is_connected()){
        bt_state_info.connect_info = 2;
    }else{
        bt_state_info.connect_info = 0;
    }

    if(bt_nor_get_link_info(link_addr)){
        tuya_bt_link_addr_load(last_link_addr);
        if(memcmp(link_addr,last_link_addr,6)){
            bt_state_info.dev_info &= ~BIT(0);
        }else{
            bt_state_info.dev_info |= BIT(0);
        }
    }else{
    ...
    TUYA_APP_LOG_DEBUG("bt_state_info.connect_info:%d\n",bt_state_info.connect_info);
    TUYA_APP_LOG_DEBUG("bt_state_info.dev_info:%d\n",bt_state_info.dev_info);
    TUYA_APP_LOG_INTERNAL_HEXDUMP_INFO("bt_state_info.mac:", bt_state_info.mac, 6);


    tuya_ble_commData_send(BT_STATE_QUREY_RESP,ack_sn, (uint8_t*)&bt_state_info, 11 + bt_state_info.name_len,encry_mode);
}
...
uint8_t tuya_ble_commData_send(uint16_t cmd,uint32_t ack_sn,uint8_t *data,uint16_t len,uint8_t encryption_mode)
{
    ...
    uint32_t send_sn = tuya_ble_send_sn++;
    ...
    TUYA_BLE_LOG_INFO("ble_commData_send len = %d , package_number = %d , protocol version : 0x%02x ",air_send_packet.encrypt_data_buf_len,package_number,TUYA_BLE_PROTOCOL_VERSION_HIGN);

...
tuya_ble_parameters_settings_t tuya_ble_current_para AT(.tuya_data);
...
#else
tuya_ble_callback_t m_cb_table[TUYA_BLE_MAX_CALLBACKS]  AT(.tuya_data);
#endif
...
tuya_ble_timer_t tuya_ble_xtimer_connect_monitor AT(.tuya_data);

#define tuya_ble_connect_monitor_timeout_ms 10000
...
{
    if(tuya_ble_timer_restart(tuya_ble_xtimer_connect_monitor,tuya_ble_connect_monitor_timeout_ms) != TUYA_BLE_SUCCESS)
    {
...
void tuya_ble_adv_change(void)
{
    ...
    uint8_t *p_buf = tuya_ble_malloc(adv_data_length + scan_rsp_data_length + 3);
    if(p_buf==NULL)
    {

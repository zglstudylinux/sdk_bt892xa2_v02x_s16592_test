...
#define BT_STATE_QUREY              0x0060
#define BT_STATE_QUREY_RESP         0x0060
...
{
    uint8_t status;   // 默认为0
    uint8_t mac[6];   // 经典蓝牙mac地址
    uint8_t ability;  // Bit0:  CTKD标志位 0-不支持  1-支持
    uint8_t dev_info; // Bit0: 配对标志， 0-未配对，1-已配对
    uint8_t connect_info;  // 0： 未连接 1： 连接中 2： 已连接
    uint8_t name_len; //蓝牙名称长度
    uint8_t name[32]; //蓝牙名称
}bt_state_info_t;

int tuya_bt_dev_info_fill(bt_state_info_t* p_bt_state);

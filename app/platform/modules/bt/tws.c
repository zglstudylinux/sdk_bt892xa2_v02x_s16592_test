...
{
    if((xcfg_cb.bt_tws_lr_mode == 9)||(xcfg_cb.bt_tws_lr_mode == 8)) {
...
#endif // USER_INEAR_DETECT_EN
#if LE_POPUP_VBAT_EN
    param[offset++] = sys_cb.loc_house_bat;
    param[offset++] = sys_cb.loc_bat;
    param[offset++] = sys_cb.loc_house_state;
#endif
#if ANC_SOFT_FADE_EN
    param[offset++] = anc_soft_loc_sta;    //将本地的anc状态同步给对耳
#endif
#if ASR_EN
...
#endif // USER_INEAR_DETECT_EN
#if LE_POPUP_VBAT_EN
    if(sys_cb.rem_house_bat != param[offset]) {
        sys_cb.rem_house_bat = param[offset];
    }
    offset++;
    tmp = sys_cb.rem_bat ^ param[offset];
    if(tmp) {
        sys_cb.rem_bat = param[offset];
    }
    offset++;
    tmp = param[offset] & 0x03;
    if(sys_cb.rem_house_state != tmp) {
        sys_cb.rem_house_state = tmp;
    }
    offset++;
#endif
#if ANC_SOFT_FADE_EN
    tmp = param[offset];
    if(tmp != anc_soft_rem_sta) {
        anc_soft_rem_sta = tmp;	    //同步对耳的anc状态
    }
    offset++;
#endif
#if ASR_EN
...
{
#if GFPS_EN
    return google_fast_pair_tws_get_data(buf);
#endif
    return 0;
}
...
{
#if GFPS_EN
    google_fast_pair_tws_set_data(data_ptr, size);
#endif
    return 0;
}

...
{
    if (dma_available_of_queue(queue) < len) {
        return DMA_ERR;
    }

    queue->len += len;

    uint32_t bytesToTheEnd = queue->size - queue->write;

    if (bytesToTheEnd > len) {
        memcpy((uint8_t*)&queue->base[queue->write], (uint8_t*)e, len);
        queue->write += len;
    } else {
        memcpy((uint8_t*)&queue->base[queue->write], (uint8_t*)e, bytesToTheEnd);
        memcpy((uint8_t*)&queue->base[0], (((uint8_t*)e) + bytesToTheEnd), len - bytesToTheEnd);
        queue->write = len - bytesToTheEnd;
    }
...
{
    if (dma_length_of_queue(queue) < len) {
        return DMA_ERR;
    }
    ...
    if (e != NULL) {
        ...
        uint32_t bytesToTheEnd = queue->size - queue->read;

        if (bytesToTheEnd > len) {
            memcpy((uint8_t*)e, (uint8_t*)&queue->base[queue->read], len);
            queue->read += len;
        } else {
            memcpy((uint8_t*)e, (uint8_t*)&queue->base[queue->read], bytesToTheEnd);
            memcpy((((uint8_t*)e) + bytesToTheEnd), (uint8_t*)&queue->base[0], len - bytesToTheEnd);
            queue->read = len - bytesToTheEnd;
        }
    } else {
        queue->read = (queue->read + len) % queue->size;
    }
...
int32_t dma_peek_queque(DMA_QUEUE* queue, uint32_t len_want, DMA_ITEM_TYPE** e1, uint32_t* len1, DMA_ITEM_TYPE** e2,
                        uint32_t* len2)
{
    if (dma_length_of_queue(queue) < len_want) {
        return DMA_ERR;
    }

    *e1 = &(queue->base[queue->read]);

    if ((queue->write > queue->read) || (queue->size - queue->read >= len_want)) {
        *len1 = len_want;
        *e2   = NULL;
        *len2 = 0;
        return DMA_OK;
    } else {
        *len1 = queue->size - queue->read;
        *e2   = &(queue->base[0]);
        *len2 = len_want - *len1;
        return DMA_OK;
    }
...
int32_t dma_peek_queue_2_buf(DMA_QUEUE* queue, DMA_ITEM_TYPE* e, uint32_t len)
{
    ...
    status = dma_peek_queque(queue, len, &e1, &len1, &e2, &len2);
...
int32_t dma_dump_queue(DMA_QUEUE* queue)
{
    ...
    while (len > 0) {
        ...
        DMA_TRACE(stderr, "-0x%x", e);
        DMA_TRACE(stderr, "-%c", e);
...
void dma_get_random_string(char* rand_string, int32_t len)
{
    ...
    if ((NULL == rand_string) || (len <= 1)) {
        DMA_TRACE("[%s] rand_string is NULL\n", __func__);
        return;
    }
    ...
    rand_string[len - 1] = '\0';
    DMA_TRACE("[%s] %s\n", __func__, rand_string);
}

uint32_t dma_merge_5strings(char* new_string, char* string1, char* string2, const char* string3, const char* string4,
                            char* string5)
{
    uint32_t char_num;
    ...
    DMA_TRACE("--merge 5 num %d -- %s\n", char_num, new_string);
    return char_num;
}


uint32_t dma_merge_4strings(char* new_string, char* string1, const char* string2, const char* string3,
                            char* string4)
{
    uint32_t char_num;
...
void dma_convert_byte_to_char(void* byte, char* sha256_char, uint32_t len)
{
    ...
    while (len--) {
        sprintf(sha256_char + count * 2, "%02x", *(unsigned char*)byte++);
        count++;
    }
}
